...
public class Sblock extends Block{
    ...
    public void run() {
        super.run();
        //Board.getBoardW()-mainSize
        
        while(true){
        System.out.println("jeste");
        if(block1.getPoint().x>270 || 
                    block2.getPoint().x>270 || 
                    block3.getPoint().x>270 ||
                    block4.getPoint().x>270){
                
                block1.getPoint().x=block1.getPoint().x-mainSize;
                block2.getPoint().x=block2.getPoint().x-mainSize;
                block3.getPoint().x=block3.getPoint().x-mainSize;
